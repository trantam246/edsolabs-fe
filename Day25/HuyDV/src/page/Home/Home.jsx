...
const Home = () => {
    ...
    const SwitchComponent = (LsTask, LsTag) => {
        switch (swCompo) {
            case 'timework':
                return <TimerWork size={size} handleLoadMore={handleLoadMore} handleClickListsDelete={handleClickListsDeletes} handleClickList={handleClickList} LsTask={LsTask} LsTag={LsTag} />
            case 'report':
    ...
    useEffect(() => {
        localStorage.getItem('user') ? history.replace("/home") : history.replace("/")
