...
const Report = ({ LsTask, LsTag }) => {
        ...
        const percentTags = [
          {
            id: 1,
            name: 'Online',
            time: (percentTag.online / sumTagMinute) * 100,
          },
          {
            id: 2,
            name: 'Meeting',
            time: (percentTag.meeting / sumTagMinute) * 100,
          },
          {
            id: 3,
            name: 'Training',
            time: (percentTag.training / sumTagMinute) * 100,
          },
          {
            id: 4,
            name: 'Coding',
            time: (percentTag.coding / sumTagMinute) * 100,
          },
        ];
  ...
  const getTotalTimer = (listDate) => {
    ...
    return listDate().reduce((listTotal, dateTimer) => {
      return (listTotal =
        listTotal +
        listTasks.reduce((total, item) => {
          const date = item.date.split(' ')[0];
          if (date === dateTimer) {
            return (total =
              total +
              item.tasks.reduce((sum, item) => {
                    ...
                    arr.forEach((i) => {
                      ...
                      if (isArr) {
                        if (i.id === e.id) {
                          i.time =
                            i.time +
                            changeTimer(tasks.time_spent) / listTag.length;
                        }
                      } else {
                        arr.push({
                          id: e.id,
