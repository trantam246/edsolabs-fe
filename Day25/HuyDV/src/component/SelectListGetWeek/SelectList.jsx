...
import { List, ListItem, ListItemText, makeStyles, Popper } from '@material-ui/core'
...
const SelectList = props => {
    ...
    return (
        <>
            <ButtonIcon onClick={handleClick}>{props.listSelect} <IoMdArrowDropdown /></ButtonIcon>
            <Popper className={classes.popper} id={id} open={open} anchorEl={anchorEl}>
                <List component="nav" className={classes.list} aria-label="contacts">
                    {props.arrList.map((item, index) => {
                        if (item === 'Date range') {
                            return (<ListItem onClick={handleClickDR} button key={index + 1}>
                                {
                            ...
                            return (<ListItem onClick={() => { handleClick(); handleGetSelecter(item) }} button key={index + 1}>
                                {
                                    <ListItemText primary={item} />
                                }
                            </ListItem>)
