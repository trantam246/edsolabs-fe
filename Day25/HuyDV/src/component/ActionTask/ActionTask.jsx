...
const ActionTask = ({ handleClickList, handleClickPlay, play }) => {
    ...
    const fulDateDay = `${newDate.getFullYear()}-${Number((newDate.getMonth() + 1)) >= 10 ? (newDate.getMonth() + 1) : '0' + (newDate.getMonth() + 1)}-${Number((newDate.getDate())) >= 10 ? (newDate.getDate()) : '0' + (newDate.getDate())} ${newDate.getHours()}:${newDate.getMinutes()}`;
    ...
    const handleChecked = (value) => {
        ...
        const getTrue = getTag.filter((item) => (item.active === true)).map((item) => item.id)
        setTagList(getTrue);
    }
    ...
    const renderListTag = getTag.map((item) => {
        return <ListItem key={item.id} role={undefined} dense button>
            <ListItemIcon>
            ...
            <ListItemText id={`checkbox - list - label - ${item.name} `} primary={` ${item.name} `} />
        </ListItem>
    })
    ...
    const handlePushData = async () => {
        ...
        const setTask = setTasker(uuid(), valueInput, tagList, formatTime(timer), 1);
    ...
    return (
        <ActionTasked>
            ...
            <GroupItem>
                ...
                <span className={classes.count}><CountTimer timer={timer} /></span>
                {play ? <FaStopCircle onClick={async () => { playButton() }} className={classes.tag} /> : <AiFillPlayCircle onClick={() => { handleClickPlay(); handleClickButtonPlay() }} className={classes.tag} />}
            </GroupItem>

            <Popper id={id} open={open} anchorEl={anchorEl}>
                <List className={classes.listDown}>
                    {
                        [
                            renderListTag
                        ]
